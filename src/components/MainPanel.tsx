      ...
      {!isComplete ? <>
          ...
          <div className="mb-6">
            ...
            <p className="text-slate-500 dark:text-slate-400 mt-1">
              Mission: Complete your WMS with all required information
            </p>
          </div>
          {/* Progress Steps */}
          <div className="flex justify-between mb-8 px-2">
            {steps.map(step => <div key={step.id} className="flex flex-col items-center" onClick={() => step.id <= currentStep && setCurrentStep(step.id)}>
                <div className={`w-10 h-10 rounded-full flex items-center justify-center mb-2 cursor-pointer
                    ${step.id < currentStep ? 'bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400' : step.id === currentStep ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'}`}>
                  {step.id < currentStep ? <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
